...
class Animal {
    constructor(id = null, pastureId, breed, age, gender) {
        this.id = id;
        this.pastureId = validatePastureId(pastureId);
    ...
    createRegistry(date, isVaccinate) {
        if (!(date instanceof Date)) {
            throw new Error("Para crear el registro debes introducir una instancia de Date")
        }
        if (!(isVaccinate instanceof IsVaccinated)) {
            throw new Error("Para crear el registro debes introducir una instancia de IsVaccinated")
        }
